...
def _make_agent_response(**overrides):
    defaults = {
        "answer": "answer",
        "metrics": AgentRunMetrics(total_turns=0),
...
class TestRunAgentForDatasetItem:
    ...
    def test_passes_question_to_invoke_agent(self, mock_invoke):
        mock_invoke.return_value = _make_agent_response(
            metrics=AgentRunMetrics(total_turns=1),
        )
    ...
    @patch("evaluate_agent_on_dataset.invoke_agent_with_user_message")
    def test_returns_serialized_output(self, mock_invoke):
        mock_invoke.return_value = _make_agent_response(
            answer="the answer is 4",
            metrics=AgentRunMetrics(total_turns=3),
        )
    ...
    @patch("evaluate_agent_on_dataset.invoke_agent_with_user_message")
    def test_passes_file_path_to_invoke_agent(self, mock_invoke):
        mock_invoke.return_value = _make_agent_response()
    ...
    @patch("evaluate_agent_on_dataset.invoke_agent_with_user_message")
    def test_passes_none_file_path_when_empty(self, mock_invoke):
        mock_invoke.return_value = _make_agent_response()
